...
<?php
	...
	if(isset($_POST['arrayDiasDaSemana'])){
	
		$arrayDiasDaSemana = $_POST['arrayDiasDaSemana'];
		
		foreach($arrayDiasDaSemana as $diaDaSemanaApagar){
			
			echo "Vou tentar apagar eventos de " . $diaDaSemanaApagar . "<br>";
			...
			for ($i = 0; $i < 7; $i++) { 
				...
				if($diaSemanaAtual == $diaDaSemanaApagar){ // verifica se o dia da semana atual e igual ao dia recebido como parametro
			...
			// DEFINE O PERIODO DE REMOCAO (Dia todo)
			$diaAtualRemocao = ($dataDoEvento. 'T00:00:00.000z'); // define o inicio do dia da semana
			$diaFinalRemocao = ($dataDoEvento.'T23:59:59.000z'); // define o final do dia da semana
	...
	} // if isset arrayDiasDaSemana
	else
		echo "Nao recebi o array de eventos a excluir do Google!";
} // if ($client->getAccessToken())
?>
